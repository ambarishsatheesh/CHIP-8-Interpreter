...
SDL_Layer::SDL_Layer(const char* title, int winWidth, int winHeight, int textureWidth, int textureHeight)
{
	...
	SDL_SetRenderDrawColor(renderer, 143, 145, 133, 255);
...
SDL_Rect SDL_Layer::ScanLines(int topLeftX, int topLeftY, int rectWidth, int rectHeight)
{
	SDL_Rect rect;
	rect.x = topLeftX;
	rect.y = topLeftY;
...
void SDL_Layer::Update(const void* buffer, int pitch, int winWidth, int winHeight)
{
	...
	for (int topLeftY = 0; topLeftY < winHeight; topLeftY += 4)
	{
		...
		SDL_RenderFillRect(renderer, &ScanLines(0, topLeftY, winWidth, 2));
	}
	SDL_RenderPresent(renderer);
